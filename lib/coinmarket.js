...
function pairData(jsonData, pair) {
  // Filters only recent data
  return jsonData.filter(function (obj) {
    if (obj.pair === pair) {
      return obj
...
function main() {
  ...
  var CoinData = getCoinData(coin)
  CoinData.then(function (data) {
    console.log('Initialized user details for ' + coin + ' and pair ' + pair)
    ...
    JSONCoinData = pairData(JSONCoinData, pair)
...
module.exports.pairData = pairData
